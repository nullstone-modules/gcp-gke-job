...
locals {
  app_metadata = tomap({
    ...
    job_definition_namespace = kubernetes_config_map_v1.job_definition.metadata[0].namespace
    job_definition_name      = kubernetes_config_map_v1.job_definition.metadata[0].name
  })
}
